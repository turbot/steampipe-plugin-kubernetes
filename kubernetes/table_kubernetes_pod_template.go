...
func tableKubernetesPodTemplate(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Description: "PodTemplates are specifications for creating Pods, and are included in workload resources such as Deployments, Jobs, and DaemonSets.",
		Get: &plugin.GetConfig{
			KeyColumns: plugin.AllColumns([]string{"name", "namespace"}),
			Hydrate:    getPodTemplate,
		},
		List: &plugin.ListConfig{
			Hydrate: listPodTemplates,
		},
...
func listPodTemplates(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	logger := plugin.Logger(ctx)
	logger.Error("listPodTemplates")
	...
	podTemplates, err := clientset.CoreV1().PodTemplates("").List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}

	for _, podTemplate := range podTemplates.Items {
		d.StreamListItem(ctx, podTemplate.Template)
	}

	return nil, nil
}

func getPodTemplate(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	logger := plugin.Logger(ctx)
	logger.Trace("getPodTemplate")
...
func transformPodTemplateTags(_ context.Context, d *transform.TransformData) (interface{}, error) {
	obj := d.HydrateItem.(v1.PodTemplateSpec)
